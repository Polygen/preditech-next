...
export default function Page() {
  useEffect(() => {
    if (typeof window !== 'undefined' && window['init' + 'registerjs']) {
      window['init' + 'registerjs']();
    }
  }, []);
...
<div class="container">
  <div class="auth-container reveal">
    ...
    <h2 style="font-size: 24px; margin-bottom: 8px;">Kayıt Ol</h2>
    <p style="color: var(--text2); font-size: 14px; margin-bottom: 32px;">Preditech ailesine katılın</p>
    ...
    <form style="text-align: left;" onsubmit="handleLogin(event)">
      <div class="form-group" style="margin-bottom: 16px;">
        <label style="display: block; margin-bottom: 8px; font-size: 12px; color: var(--text2);">Ad Soyad</label>
        <input type="text" id="name" class="sporty-input" style="width: 100%; box-sizing: border-box;" placeholder="Noyan S." required>
      </div>
      <div class="form-group" style="margin-bottom: 16px;">
      ...
      <div class="form-group" style="margin-bottom: 24px; display: flex; align-items: center; gap: 8px;">
        <input type="checkbox" id="terms" required style="accent-color: var(--accent);">
        <label for="terms" style="font-size: 11px; color: var(--text2);">Kullanım koşullarını okudum ve kabul ediyorum.</label>
      </div>
      <p id="error-msg" style="color: var(--danger); font-size: 12px; margin-bottom: 12px; display: none;">Hata oluştu.</p>
      <button type="submit" class="btn btn-primary sporty-btn" style="width: 100%; justify-content: center; background: var(--accent); color: #000; font-weight: 800;">KAYIT OL</button>
    </form>
    
    <p style="margin-top: 24px; font-size: 13px; color: var(--text2);">
      Zaten hesabınız var mı? <a href="/login" style="color: var(--accent); text-decoration: none; font-weight: 600;">Giriş Yap</a>
    </p>
  </div>
</div>
...
` }} />
      ...
      <Script src="/js/register.js" strategy="lazyOnload" onLoad={() => { if(window['init' + 'registerjs']) window['init' + 'registerjs'](); }} />
    </>
  );
}
