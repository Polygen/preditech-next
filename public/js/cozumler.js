...
window.openCozumModal = function(index) {
  ...
  let modalDesc = document.getElementById('mDesc');
  modalDesc.innerHTML = s.d;
  
  if(s.links && s.links.length > 0) {
    let linksHtml = s.links.map(l => `
<a href="${l.url}" style="display:flex; align-items:center; gap:12px; padding:12px; background:var(--surface2); border:1px solid var(--border); border-radius:12px; text-decoration:none; color:var(--text); transition:all 0.3s ease; cursor:pointer;" onmouseover="this.style.borderColor='var(--accent)'; this.style.transform='translateY(-2px)';" onmouseout="this.style.borderColor='var(--border)'; this.style.transform='none';">
  <div style="width:50px; height:50px; background:var(--bg); border-radius:8px; display:flex; align-items:center; justify-content:center; border:1px solid rgba(255,215,0,0.1); padding:4px;">
    <img src="${l.img}" alt="${l.name}" style="max-width:100%; max-height:100%; object-fit:contain;">
  </div>
  <div style="flex:1;">
    <div style="font-weight:700; font-size:14px; color:var(--text); margin-bottom:2px;">${l.name}</div>
    <div style="font-size:11px; color:var(--text2); text-transform:uppercase; letter-spacing:1px; background:rgba(255,255,255,0.05); display:inline-block; padding:2px 6px; border-radius:4px;">${l.type}</div>
  </div>
  <i class="ph ph-caret-right" style="color:var(--accent); font-size:18px;"></i>
</a>`).join('');
    
    modalDesc.innerHTML += `<div style="margin-top:24px; padding-top:24px; border-top:1px solid rgba(255,215,0,0.1);">
      <h4 style="font-size:13px; font-weight:800; color:var(--text2); letter-spacing:1px; margin-bottom:16px; text-transform:uppercase;">Bu Çözümün Sunulduğu Cihazlar:</h4>
