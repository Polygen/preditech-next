...
export default function OdemePage() {
  ...
  useEffect(() => {
    ...
    // Load Addons
    const storedAddonsStr = localStorage.getItem('preditech_addons');
    if (storedAddonsStr) {
      try {
        const storedAddons = JSON.parse(storedAddonsStr);
        setSelectedAddons(storedAddons);
        const storedTotal = parseInt(localStorage.getItem('preditech_addonsTotal') || '0');
        setAddonsTotal(storedTotal);
      } catch(e) {
          ...
          <div>
            <div className="checkout-card" style={{position: 'sticky', top: '120px'}}>
              ...
              {selectedAddons.map((addon, index) => (
                <div className="summary-item" key={'sum-'+index}>
